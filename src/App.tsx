...
export function App(){
  ...
  const scheduleInfo=(data.ScheduleInfo||[]).filter((x:any)=>x.termId===term);
  const matchingCourses=currentCourses.filter((c:any)=>Schedule.searchText(c).includes(search.trim()));
  ...
  async function copyReminder(){
    ...
    const text=Schedule.reminder(upcoming,s(data,'className'),dateText(upcoming.date));
  ...
  function courseCard(c:any){return <article key={c.id} className={'course-row '+(c.status==='cancelled'?'cancelled':'')}>
    <div className="date-tile"><span>{c.date.slice(5,7)} 月</span><b>{c.date.slice(8,10)}</b><small>{['週日','週一','週二','週三','週四','週五','週六'][new Date(c.date+'T12:00:00Z').getUTCDay()]}</small></div>
    <div className="course-main"><div className="course-title"><h3>{c.title}</h3>{c.status==='cancelled'&&<Pill tone="gray">停課</Pill>}</div>{c.scripture&&<p>{c.scripture}</p>}<div className="meta-line"><span><Clock size={15}/>{c.startTime}–{c.endTime}</span>{!c.periods?.length&&c.teacher&&<span><Users size={15}/>{c.teacher}</span>}{c.dutyTeacher&&<span className="duty-teacher"><Users size={15}/>值星：{c.dutyTeacher}</span>}</div>
    {!!c.periods?.length&&<ol className="course-periods" aria-label="課程時段">{c.periods.map((p:any,i:number)=><li key={i}><div className="period-heading"><span>{p.label}</span><time>{p.startTime}–{p.endTime}</time></div><strong>{p.title||'尚未安排'}</strong><small>{p.teacher?p.teacherLabel+'：'+p.teacher:'教員尚未安排'}</small></li>)}</ol>}
    <details><summary>課程內容與準備事項</summary><dl><dt>本週金句</dt><dd>{c.verse||'教員尚未填寫'}</dd>{!c.periods?.length&&<><dt>詩歌</dt><dd>{c.song||'尚未安排'}</dd></>}<dt>攜帶物品</dt><dd>{c.materials||'依教員通知'}</dd>{c.notes&&<><dt>備註</dt><dd>{c.notes}</dd></>}</dl>{c.resourceUrl&&<a href={c.resourceUrl} target="_blank" rel="noreferrer" className="text-link">開啟教材 <ExternalLink size={14}/></a>}</details></div>
  </article>;}
  ...
  if(page==='home')content=<>
    ...
    <div className="home-top"><section className="next-class panel"><div className="between"><span className="eyebrow">下一次上課</span><Pill tone="light">班級課程</Pill></div>{upcoming?<><div className="next-date">{dateText(upcoming.date)}<span>{upcoming.startTime}–{upcoming.endTime}</span></div><h2>{upcoming.title}</h2><p>{upcoming.scripture}</p><div className="prepare"><BookOpen size={21}/><div><small>記得帶上</small><strong>{upcoming.materials||'依教員通知'}</strong></div></div><button className="button cream" onClick={()=>go('schedule')}>查看完整課表 <ChevronRight size={17}/></button></>:<Empty title="下一次課程尚未公布">教員更新課表後會顯示在這裡。</Empty>}</section>
  ...
  else if(page==='schedule')content=<><Heading eyebrow="每次相聚，一起學習" title="班級課表" actions={termSelect()}>課程與準備事項，以教員最新更新為準。</Heading>
    {scheduleInfo.map((info:any)=><section className="season-card" key={info.id}><span className="eyebrow">本季課程</span><h2>{info.title||termName}</h2>{info.goal&&<div className="season-goal"><BookOpen size={20}/><div><h3>本季目標</h3><p>{info.goal}</p></div></div>}{isStaff&&info.teacherNotes&&<details className="teacher-notes"><summary>教員工作提醒</summary><p>{info.teacherNotes}</p></details>}</section>)}
    <div className="toolbar"><input className="search" aria-label="搜尋課程" placeholder="搜尋課程、經文、教員或值星…" value={search} onChange={e=>setSearch(e.target.value)}/>{isStaff&&<button className="button secondary" onClick={copyReminder}><Copy size={16}/>複製下次上課提醒</button>}</div><div className="course-list">{matchingCourses.map(courseCard)}{!currentCourses.length?<Empty icon={CalendarDays} title="這個學期尚未建立課表"/>:!matchingCourses.length&&<Empty title="沒有符合的課程">試試其他課程名稱、教員或值星。</Empty>}</div></>;
  else if(page==='albums'||page==='admin/albums')content=albumsView(page==='admin/albums');
  else if(page==='events')content=<><Heading eyebrow="一起參與，一起留下回憶" title="近期活動" actions={termSelect()}/>{scheduleInfo.filter((info:any)=>info.activities).map((info:any)=><section className="season-card season-activities" key={info.id}><span className="eyebrow">{info.title||termName}</span><h2><CalendarDays size={21}/>本季活動</h2><p>{info.activities}</p></section>)}<div className="events-grid">{data.Events.filter((e:any)=>e.published).sort((a:any,b:any)=>a.date.localeCompare(b.date)).map((e:any)=><article className="event-card" key={e.id}><div className="event-top"><CalendarDays size={26}/><Pill tone={e.date<taipeiToday()?'gray':'green'}>{e.date<taipeiToday()?'活動已結束':'活動預告'}</Pill></div><h2>{e.title}</h2><p className="meta-line"><CalendarDays size={16}/>{dateText(e.date)}　{e.time}</p><p className="meta-line"><MapPin size={16}/>{e.location||'地點待公布'}</p><p className="event-body">{e.body}</p>{e.url&&<a className="button secondary" href={e.url} target="_blank" rel="noreferrer">報名或查看附件 <ExternalLink size={16}/></a>}</article>)}</div>{!data.Events.some((e:any)=>e.published)&&!scheduleInfo.some((info:any)=>info.activities)&&<Empty icon={Heart} title="近期活動準備中"/>}</>;
